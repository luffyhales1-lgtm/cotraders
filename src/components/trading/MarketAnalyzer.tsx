...
export const MarketAnalyzer: React.FC<MarketAnalyzerProps> = ({ onSignals }) => {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-cyan-500/30 bg-gradient-to-br from-slate-900 via-indigo-950/70 to-slate-900 p-6 md:p-8 shadow-2xl mb-8">
      ...
      <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-5">
        <div className="flex items-start gap-4">
          <div className="h-14 w-14 shrink-0 rounded-2xl bg-cyan-500/15 border border-cyan-500/40 flex items-center justify-center shadow-lg shadow-cyan-500/10">
            <Radar className={`h-7 w-7 text-cyan-400 ${analyzing ? 'animate-spin' : ''}`} />
          </div>
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h2 className="text-2xl md:text-3xl font-black tracking-tight text-white">Analyze Whole Market</h2>
              <Badge className="bg-cyan-500/20 text-cyan-300 border-cyan-500/40 text-[10px] font-bold gap-1">
                <Flame className="h-3 w-3 text-amber-400" /> ALL 21 STRATEGIES
              </Badge>
            </div>
            <p className="text-sm text-slate-300/90 mt-1 max-w-2xl leading-relaxed">
        ...
        <Button
          onClick={runAnalysis}
          disabled={analyzing}
          className="bg-gradient-to-r from-cyan-500 to-indigo-500 hover:from-cyan-400 hover:to-indigo-400 text-slate-950 font-black px-7 py-6 rounded-2xl shadow-lg shadow-cyan-500/25 gap-2 text-base shrink-0"
        >
      ...
      {overview && (
        <div className="relative z-10 mt-6 space-y-5">
          ...
          <div className="flex flex-col sm:flex-row items-stretch gap-4">
            <div className="flex-1 rounded-2xl border border-slate-800 bg-slate-950/70 p-5 flex items-center justify-between">
              <div>
                <span className="text-[11px] uppercase tracking-wider text-slate-400 font-bold">Net Market Bias</span>
              ...
              <div className="text-right">
                <span className="text-[11px] uppercase tracking-wider text-slate-400 font-bold">Strength</span>
                <div className="text-3xl font-black text-white tabular-nums">{overview.biasStrengthPct}%</div>
                <span className="text-[11px] text-slate-500 font-mono">as of {overview.timestamp}</span>
              </div>
            </div>

            <div className="flex-1 rounded-2xl border border-slate-800 bg-slate-950/70 p-5 grid grid-cols-2 gap-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
            <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-3">
              <span className="text-[10px] uppercase tracking-wider text-slate-400 font-bold block">Coverage this scan</span>
              <span className="text-sm font-bold text-slate-100">
                {overview.scannedCount} instruments scanned{universeSize ? ` · ${universeSize} in rotation` : ''}
              </span>
            </div>
            <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-3">
              <span className="text-[10px] uppercase tracking-wider text-slate-400 font-bold block">Fresh signals found</span>
              <span className="text-sm font-bold text-cyan-400">{overview.signalCount} live setups</span>
            </div>
            <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-3">
              <span className="text-[10px] uppercase tracking-wider text-slate-400 font-bold block">BTC/USDT read</span>
              <span className="text-sm font-bold text-slate-100">{overview.btcTrend ?? 'No BTC trigger this scan'}</span>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {overview.topStrategies.map(s => (
                  <Badge key={s.name} variant="outline" className="border-indigo-500/40 text-indigo-300 text-[11px] font-bold">
                    {s.name} · {s.count}
                  </Badge>
                ))}
              </div>
                ...
                {overview.strongest.map(sig => (
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <Badge className={sig.type === 'LONG' ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' : 'bg-rose-500/20 text-rose-400 border-rose-500/30'}>
                        {sig.type}
                      </Badge>
                    ...
                    <div className="text-right shrink-0">
                      <span className="text-sm font-black text-cyan-400 tabular-nums block">{sig.confidenceScore ?? '—'}/100</span>
                      <span className="text-[10px] text-slate-400 font-mono">
...
const StatBox: React.FC<{ label: string; value: string; tone: string; icon: React.ReactNode }> = ({ label, value, tone, icon }) => (
  <div className="rounded-xl bg-slate-900/60 border border-slate-800/60 p-3">
    ...
    <span className={`text-lg font-black tabular-nums ${toneMap[tone] ?? 'text-slate-100'}`}>{value}</span>
  </div>
);
